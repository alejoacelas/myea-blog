...
9. **Regenerate the LLM text files.** Run `uv run scripts/build_llms_txt.py` — it re-reads every Doc linked from `index.html` and rewrites `llms.txt` (index) and `llms-full.txt` (full text of all posts, for pasting into AI chatbots). It aborts if any body mentions "Jojo"; fix the Doc (rewrite to "Robin") and re-run. Run this whenever a post is added, removed, or substantially re-edited.
10. **Deploy + commit.** `vercel deploy --prod`, then a git commit summarizing the new posts.
