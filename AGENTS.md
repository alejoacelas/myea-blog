...
- `public/llms.txt` / `public/all.txt`: LLM-convention text mirrors of the blog, generated by `uv run scripts/build_llms_txt.py` (see sweep step 9). Never edit them by hand.
...
9. **Regenerate the LLM text files.** Run `uv run scripts/build_llms_txt.py` — it re-reads every Doc linked from `index.html` and rewrites `llms.txt` (index) and `all.txt` (full text of all posts, for pasting into AI chatbots). It aborts if any body mentions "Jojo"; fix the Doc (rewrite to "Robin") and re-run. Run this whenever a post is added, removed, or substantially re-edited.
