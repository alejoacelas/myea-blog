...
- Plain HTML, no framework, no build step. Everything served lives in `public/` (set as `outputDirectory` in `vercel.json`) — pages, `style.css`, `analytics.js`, and the llms files. URLs map 1:1 to `public/` paths. `index.html` below always means `public/index.html`. Files outside `public/` and `api/` are never served.
- `public/llms.txt` / `public/llms-full.txt`: LLM-convention text mirrors of the blog, generated by `uv run scripts/build_llms_txt.py` (see sweep step 9). Never edit them by hand.
- Self-hosted click analytics: `analytics.js` beacons page views and post-link clicks to `api/hit.js`, which writes one Vercel Blob per hit under `hits/<kind>/<id>/`; `api/counts.js` aggregates by listing, and `/stats` renders the public counts. Blob store: `myea-clicks`, token in the `BLOB_READ_WRITE_TOKEN` project env var (pull with `vercel env pull .env.local`).
